...
\begin{abstract}
  ...
  number of predicted outcomes tends to be small, the number of
  forecasting models in the literature can be large, and missing
  predictions for component models are not uncommon. In these
  circumstances, EBMA models may miss-weight components, undermining
  the advantages of the ensemble approach to prediction.  Small
  samples increase the probability that a model will make accurate
  (inaccurate) predictions by chance, thus causing model weights to be
  too large (small).  In this article, we explore these issues and
  introduce a ``wisdom of the crowds'' parameter to the standard EBMA
  framework, which improves its performance. Specifically, we show
  that this solution improves the accuracy of EBMA forecasts in
  predicting the 2012 US presidential election and the US unemployment
  rate.
\end{abstract}
...
\citep{Wright:2008}, ice formation \citep{Berrocal:2010}, visibility
\citep{Chmielecki:2010}, climatology \citep{Min:2007, Smith:2009}, and
hydrology \citep{Zhang:2009}.

Though EBMA is already in use, research to improve upon the basic
...
number of contexts
\ifthenelse{\boolean{blind}}{\citep[e.g.,][]{Bartels:2001, Gill:2004,
    Imai:2004, Author:2020b}}{\citep[e.g.,][]{Bartels:2001, Gill:2004,
    Imai:2004, Montgomery:2010}}. A more detailed discussion of the
basic EBMA model extended here is provided in
\ifthenelse{\boolean{blind}}{\citet{Author:2020b}}{\citet{mhw:2012}}.
...
model then generates out-of-sample predicitions for each for the
calibration period $T$, which represents the time period used to
calculate model weights. It is then possible to generate true ensemble
out-of-sample forecasts ($\mathbf{f}_k^{t^\ast}$) for observations in
...
across components.\footnote{The model presented
  ...
  and binary outcomes \citep{Sloughter:2007, Sloughter:2010,
...
result from our proposed modification.  We then apply our method to,
first, the prediction of the US unemployment rate, and, second, to the
prediction of the 2012 US presidential
election.\footnote{\ifthenelse{\boolean{blind}}{All files needed for
    ...
    Comprehensive R Archiving Network (CRAN) \citep{Montgomery:2012d}.}}
...
in \ifthenelse{\boolean{blind}}{\citet{Author:2020b}}{\citet{mhw:2012}}, missing observations in component model
...
rewrite the EBMA algorithm further to make estimation of $c$
possible.
