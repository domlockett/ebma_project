...
### unix systems, parallel backend
library(doMC)

###windows, parallel backend
#library(doSNOW)
...
summary(output)
#save(output, file="sim_output.rda")
...
wireframe(tryThis~for_pred[,1]+for_pred[,2],xlab=expression(frac("K",n[T])), ylab="C", zlab="CRPS", drape=TRUE,lwd=0,col.regions=col,scales = list(arrows=FALSE, cex= 0.7, col = "black",font = 1, tck = 0.5),colorkey=F)
dev.off()
