...
pred12<-as.data.frame(matrix(ncol=11,nrow=1,c(2012,51.9,49.5,50.6,52.0,47.5,48.2,53.8,47.9,52.6,46.9))) # changed cuzan to their preferred forecast

### create data file for replication 
for_rep = .reduced[,-3]
names(pred12) = names(for_rep)
for_rep = rbind(for_rep,pred12)
for_rep = as.data.frame(for_rep)
names(for_rep)
for(i in 1:dim(for_rep)[2]){
	for_rep[,i] = as.numeric(for_rep[,i])
}
write.csv(for_rep,file="~/Dropbox/EBMA/IJF_Replication/presidential_data.csv",row.names = FALSE)

.FD <- makeForecastData(.predCalibration=.reduced[,-c(1:3)]
                          ,.outcomeCalibration=.reduced[,2]
                          ,.predTest=pred12[,-c(1,2)] 
                          ,.outcomeTest=51.9
...
setwd("~/Dropbox/EBMA/EBMA2JOFSubmission/")
