...
# PRE
  ...
  names(models.brier) <- names(models.pre) <- names(models.perc) <- names(models.auc) <- names(W) <- colnames(model.params) <- colnames(pp.raw)<-model.names
...
plot.Ensemble.logit<-function(obj){
    ...
    separationplot.icews(pred=obj$pred.prob, actual=obj$dv, newplot=F, heading="BMA", type="line")
    for(i in 1:num.models){
      separationplot.icews(pred=obj$model.pps[,i], actual=obj$dv, newplot=F, heading=obj$model.names[i], type="line")
    }
  }
...
print.Ensemble.logit<-function(obj, digits=3){
    ...
    out<-matrix(NA, ncol=7, nrow=(num.models+1))
    dimnames(out) <- list(c(obj$model.names, "BMA"), c("Weight", "Constant", "Predictor", "AUC", "PRE", "Brier", "% Correct"))
