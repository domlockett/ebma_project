...
in.sample <- icews[icews$year>=1999 & icews$year<=2008,] # 1999-2008
out.sample <- icews[icews$year==2009| icews$year==2010,] # 2009-2010

model.polisci <-lmer(insurgency ~ xconst + USAcoop.l3 +antiGovtviolence.l3 + (1|country), data = in.sample,family="binomial")
summary(model.polisci)
in.m1 <- fitted(model.polisci, newdata=in.sample)
out.m1 <- predict.lmer(model.polisci, newdata=out.sample)

glm.model.econ <-glm(insurgency ~ pop.l3 + gdpgrowth.l3 + USAcoop.l3 +nminorities, data = in.sample, family="binomial")
...
in.m3 <- fitted(model.lmer.3)
out.m3 <- predict.lmer(model.lmer.3, newdata=out.sample)
...
model.lmer.alltheway<-lmer(insurgency~ pop.l3 + USAcoop.l3 + antiGovtviolence.l3 + parcomp + xconst + (1   | country), data=in.sample, family="binomial")
summary(model.lmer.alltheway)
in.m5 <- fitted(model.lmer.alltheway)
out.m5 <- predict.lmer(model.lmer.alltheway, newdata=out.sample)

model.icews<-lmer(insurgency~ recentonsets.insurgency + lastelection + mil.conf.l3  + W.bl.std.crisisdomestic.l3 + USAconf.l3 + (-1  + gdppc.l3|country),data=in.sample, family="binomial")
in.m6 <- fitted(model.icews)
out.m6 <- predict.lmer(model.icews,newdata=out.sample)
...
Duke.in.pred <- data.frame(in.m1, in.m2, in.m3, in.m4, in.m5, in.m6, as.character(in.sample$country), in.sample$year, in.sample$month, in.sample$insurgency, stringsAsFactors=FALSE)
names(Duke.in.pred)<-c("Polisci", "Glm.Econ", "Lmer3", "Glm.simple", "Lmer.all", "Icews", "Country", "Year", "Month", "Insurgency")

Duke.out.pred <- data.frame(out.m1, out.m2, out.m3, out.m4, out.m5, out.m6, as.character(out.sample$country), out.sample$year, out.sample$month, out.sample$insurgency, stringsAsFactors=FALSE)
names(Duke.out.pred)<-c("Polisci", "Glm.Econ", "Lmer3", "Glm.simple", "Lmer.all", "Icews", "Country", "Year", "Month", "Insurgency")
