...
#simulate some data outcome based on LMER, SAE, and GLM input

predCalibration <- rbind(calibrationSample[,c(1,2,3)],calibrationSample[,c(1,2,3)],calibrationSample[,c(1,2,3)])
...
this.ensemble <- calibrateEnsemble(dat, model="logit", tol = 0.00000000000000000000001, exp=3, method = "EM", useModelParams = F)
summary(this.ensemble)

this.ensemble <- calibrateEnsemble(dat, model="logit", tol = 0.00000000000000000000001, exp=3, method = "gibbs", useModelParams = F)
summary(this.ensemble)
...
# 
# w2 = c(1,0,0)
x2 = GibbsLogit(outcomeCalibration,as.matrix(predCalibration),W,20000, burnin = 4000, thin = 3)
# 
apply(x2[["W_out"]],2,mean)
